"""Interactive commands for configuring and running the course scheduler.

This module manages configuration lifecycle operations, CRUD workflows,
scheduling options, schedule generation, and schedule export through a shared
session object.
"""
...
def add_faculty(session):
    ...
    def _mutate(cfg):
        """Apply the requested change to the editable configuration draft."""
...
def add_meeting(session):
    """Add a meeting to a selected class pattern."""
    config = session.require_config()
    patterns = _list_patterns(config)
    if not patterns:
...
def delete_meeting(session):
    """Delete a meeting while preserving pattern validity."""
    config = session.require_config()
    patterns = _list_patterns(config)
    if not patterns:
        return
